...
% Time array
t = 0:dt:total_time; % Ensure correct number of time steps
...
for i = 1:length(t)-1
   ...
   q_dot_inv = pinv(J) * error'; % Use only the Cartesian position error 
    ...
    q_dot_transpose = J' * error'; % Only Cartesian error is used
    
    % Use inverse Jacobian for integration (can switch to q_dot_transpose)
    q(:, i + 1) = q(:, i) + dt * q_dot_transpose;
end
